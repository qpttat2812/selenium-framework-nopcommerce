...
public class NotebooksPageObject extends BasePages {
	...
	public List<String> getBeforeListByProductNameAndSort(String expectedSortType) {
		waitForElementInvisibility(NotebooksPageUI.AJAX_PRODUCT_LOADING_ICON);

		List<WebElement> elements = getElements(NotebooksPageUI.ITEM_NAME_LIST);
		...
		selectDefaultItemDropdownList(NotebooksPageUI.BASE_SORT_DROPDOWNLIST, expectedSortType);
	...
	public List<Float> getBeforeListByProductPriceAndSort(String expectedSortType) {
		...
		List<WebElement> elements = getElements(NotebooksPageUI.ITEM_PRICE_LIST);
	...
	public void selectDisplayType(String expectedType) {
		waitForElementInvisibility(NotebooksPageUI.AJAX_PRODUCT_LOADING_ICON);
		selectDefaultItemDropdownList(NotebooksPageUI.DISPLAY_TYPE_DROPDOWNLIST, expectedType);
	}
	...
	public boolean isItemSizeDisplayedCorrect(int expectedSize) {
		...
		if (getElementSize(NotebooksPageUI.ITEM_DISPLAY_LIST) <= expectedSize) {
			return true;
		} else {
			return false;
		}
	...
	public boolean isNextPagingIconDisplayed() {
		waitForElementInvisibility(NotebooksPageUI.AJAX_PRODUCT_LOADING_ICON);
		return isElementDisplayed(NotebooksPageUI.NEXT_PAGING_ICON);
	}

	@Step("Verify Previous Paging icon is displayed")
	public boolean isPreviousPagingIconDisplayed() {
		waitForElementInvisibility(NotebooksPageUI.AJAX_PRODUCT_LOADING_ICON);
		waitForElementVisibility(NotebooksPageUI.PREVIOUS_PAGING_ICON);
		return isElementDisplayed(NotebooksPageUI.PREVIOUS_PAGING_ICON);
	}

	@Step("Verify Next Paging icon is undisplayed")
	public boolean isNextPagingIconUndisplayed() {
		return isElementUndisplayed(NotebooksPageUI.NEXT_PAGING_ICON);
	}

	@Step("Verify Previous Paging icon is undisplayed")
	public boolean isPreviousPagingIconUndisplayed() {
		return isElementUndisplayed(NotebooksPageUI.PREVIOUS_PAGING_ICON);
	}

	@Step("Click on 'Next' Page")
	public void clickOnNextPage() {
		waitForElementInvisibility(NotebooksPageUI.AJAX_PRODUCT_LOADING_ICON);
		clickOnElement(NotebooksPageUI.NEXT_PAGING_ICON);
	}

	@Step("Click on 'Previous' Page")
	public void clickOnPreviousPage() {
		waitForElementInvisibility(NotebooksPageUI.AJAX_PRODUCT_LOADING_ICON);
		clickOnElement(NotebooksPageUI.PREVIOUS_PAGING_ICON);
	}
}
